...
import {
  APPROVAL_OF_THE_AGENDA,
  CLOSE_OF_THE_MEETING,
  OPEN_OF_THE_MEETING,
} from '../constansts/constans';
import { TextChannel } from 'discord.js';

export const set = async (message, args, pollChannel: TextChannel) => {
  ...
  items.forEach((el) => {
    ...
    if (el.includes('|')) {
      ...
      message.channel.send(
        `Пункт "${cleanedItem[0]}" додано, але при !next: буде ${cleanedItem[1]}!`,
      );
    } else {
